...
void add_to_queue(afl_state_t *afl, u8 *fname, u32 len, u8 passed_det) {
  ...
  // PMFuzz: Set if this testcase had new PM access
  q->new_pm_access = afl->had_new_pm_access;

  // PMFuzz: Reset the value in afl
  afl->had_new_pm_access = 0;
...
/* Go over the queue and set favored for PM test cases */
void cull_queue_pm(afl_state_t *afl) {
  struct queue_entry *q;

  q = afl->queue;

  /* If a case had new PM access mark it as favored*/
  while (q) {
    if (q->new_pm_access){
      if (!q->favored) {
        q->favored = 1;
