...
int main(int argc, char **argv_orig, char **envp) {
  ...
  afl->fsrv.trace_pm_bits = afl->shm.pm_map;

  setup_post(afl);

  if (!afl->in_bitmap) memset(afl->virgin_bits, 255, MAP_SIZE);
  if (!afl->in_bitmap) memset(afl->virgin_pm_bits, 255, MAP_SIZE);
...
/* Checks for PM accesses in map, first half of the map is for PM reads, 
   second half for writes. */
u32 pm_read_count(u8 *pm_map) {
  u32 result = 0;
  for (u32 i = 0; i < MAP_SIZE; i++) {
    if ((i < MAP_SIZE/2) && pm_map[i]) {
      result++;
    }
  }
  return result;
...
/* Marks entry's pm access for favouring them over other paths */
void mark_pm_paths(struct queue_entry* q) {
  // if (pm_read_count(trace_pm_bits)) {
  //   q->pm_access_type |= PM_ACCESS_R;
  // }

  // if (pm_write_count(trace_pm_bits)) {
  //   q->pm_access_type |= PM_ACCESS_W;
  // }

  // if (q->pm_access_type != PM_ACCESS_NONE) {
  //   if (q->pm_access_type & PM_ACCESS_R) {
  //     pm_paths_found_r++;
  //   }
  //   if (q->pm_access_type & PM_ACCESS_W) {
  //     pm_paths_found_w++;
  //   }
  //   pm_paths_found++;
  // }

  // /* Record the accesses to the PM virgin bitmap, PM virgin bitmap holds 
  //    cumulative location of PM access seen until now */
  // for (u32 loc = 0; loc < MAP_SIZE; loc++) {
  //   if ((~virgin_pm_bits[loc] & trace_pm_bits[loc]) != ~virgin_pm_bits[loc]) {

  //     /* Haven't seen this access until now */
  //     q->pm_favored = 1;
  //   }

  //   /* Update the virgin map */
  //   // if (trace_pm_bits[loc]) {
  //   //   virgin_pm_bits[loc] = 1;
  //   // }
  //   // virgin_pm_bits[loc] = virgin_pm_bits[loc]|~trace_pm_bits[loc];
  // }
