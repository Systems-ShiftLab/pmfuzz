...
u8 *afl_shm_init(sharedmem_t *shm, size_t map_size, unsigned char dumb_mode) {
  ...
  shm->shm_pm_id = shmget(IPC_PRIVATE, map_size, IPC_CREAT | IPC_EXCL | 0600);

  if (shm->shm_id < 0) PFATAL("shmget() failed");
  if (shm->shm_pm_id < 0) PFATAL("shmget() failed");
  ...
  shm_pm_str = alloc_printf("%d", shm->shm_pm_id);
  ...
  if (!dumb_mode) setenv(SHM_PM_ENV_VAR, shm_pm_str, 1);

  ck_free(shm_str);
  ck_free(shm_pm_str);

  if (shm->cmplog_mode) {
    printf("Cmplog mode not enabled in PMFuzz\n");
    exit(1);
  ...
  shm->pm_map = shmat(shm->shm_pm_id, NULL, 0);

  if (shm->map == (void *)-1 || !shm->map) PFATAL("shmat() failed");
  if (shm->pm_map == (void *)-1 || !shm->pm_map) PFATAL("shmat() failed");
